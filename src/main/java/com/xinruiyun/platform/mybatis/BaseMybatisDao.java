package com.xinruiyun.platform.mybatis;

import com.xinruiyun.platform.mybatis.page.MysqlDialect;
import com.xinruiyun.platform.mybatis.page.Pagination;
...
public class BaseMybatisDao<T> extends SqlSessionDaoSupport {
	...
	public BaseMybatisDao() {
		try {
			...
			Log.e(this.getClass(),"初始化失败，继承BaseMybatisDao，没有泛型！");
		}
	}
	/**
	 * 根据Sql id 去查询 分页对象
	...
	public Pagination findByPageBySqlId(String sqlId,
		...
		Log.d(SELF, "findByPageBySqlId sql : "+sqlcode );
		...
		try {
			...
			Log.e(SELF, "jdbc.error.code.findByPageBySqlId"+e);
		}
		return page;
	}
	...
	public Pagination findPage(String sqlId, String countId,
		...
		BoundSql boundSql = c.getMappedStatement(sqlId).getBoundSql(params);
		String sqlcode = boundSql.getSql();
		Log.d(SELF, "findPage sql : "+sqlcode );
		String countCode = "";
		BoundSql countSql = null;
